...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
		...
		blackScreen.transform.SetParent(FindObjectOfType<Canvas>().transform);
    ...
    void FixedUpdate()
    {
        ...
        {
            transform.position = Vector2.Lerp(transform.position, overridenPosition, Time.deltaTime * camSpeed);
            transform.position = new Vector3(transform.position.x, transform.position.y, -10);
        }
    }

    public void OverridePos (bool overide, Vector3 pos)
    {
        if (overide)
        {
            overridden = true;
            overridenPosition = pos;
        }
        else
        {
            overridden = false;
    ...
    public void OverridePos(bool overide)
    {
        if (overide)
        {
            overridden = true;
